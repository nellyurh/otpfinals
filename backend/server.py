...
async def get_profile(user: dict = Depends(get_current_user)):
    return {
        ...
        'tier': user.get('tier', 1),
        'bvn': user.get('bvn'),
        'bvn_verified': user.get('bvn_verified', False),
        'nin': user.get('nin'),
        'nin_verified': user.get('nin_verified', False),
...
async def get_5sim_services(country: Optional[str] = None, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 5sim API returns prices directly in USD - no coin conversion needed
...
async def admin_provider_balances(admin: dict = Depends(require_admin)):
    ...
    daisysms_key = get_api_key(config, 'daisysms_api_key', DAISYSMS_API_KEY)
    smspool_key = get_api_key(config, 'smspool_api_key', SMSPOOL_API_KEY)
    fivesim_key = get_api_key(config, 'fivesim_api_key', FIVESIM_API_KEY)
...
class CreateCardRequest(BaseModel):
    brand: str = "MASTERCARD"  # MASTERCARD or VISA
...
async def get_public_branding():
    ...
    return {
        ...
        "reseller_api_base_url": config.get("reseller_api_base_url", "https://finance-portal-217.preview.emergentagent.com"),
...
async def submit_tier2_kyc(request: Tier2KYCRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Check current tier
        db_user = await db.users.find_one({'id': user['id']}, {'_id': 0})
        if db_user.get('tier', 1) >= 2:
            raise HTTPException(status_code=400, detail="Already at Tier 2 or higher")
        ...
        return {
            'success': True,
            'message': 'Tier 2 verification complete! Your limit is now ₦100,000',
            'tier': 2
        }
    except HTTPException:
        raise
...
async def verify_bvn_for_tier3(request: Tier3KYCRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Check if already BVN verified
        if db_user.get('bvn_verified'):
            return {
                'success': True,
        ...
        # Check phone match
        phone_match = request.phone.replace('+234', '0').replace('234', '0') in bvn_phone or bvn_phone in request.phone.replace('+234', '0').replace('234', '0')
...
async def verify_nin_for_tier3(request: Tier3KYCRequest, user: dict = Depends(get_current_user)):
    """Verify NIN via Payscribe lookup for Tier 3 Express KYC - no additional fee"""
    try:
        ...
        # Check if already NIN verified
        if db_user.get('nin_verified'):
            return {
                'success': True,
        ...
        errors = []
        if not name_match:
            errors.append(f"Name mismatch: Your name '{db_user.get('first_name', '')} {db_user.get('last_name', '')}' doesn't match NIN")
        if not dob_match:
            errors.append("Date of birth doesn't match NIN records")
