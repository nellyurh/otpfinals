  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {pageToggles?.enable_payscribe && (
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 border-indigo-300 shadow-sm relative">
          {/* Recommended Badge */}
          <div className="absolute -top-2.5 left-4 px-2.5 py-0.5 bg-indigo-500 text-white text-[10px] font-bold rounded-full shadow-sm">
            RECOMMENDED
          </div>
          
          <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 mt-1">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-500 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
          ...
          <div className="mb-3 sm:mb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="px-2.5 sm:px-3 py-2.5 sm:py-3 bg-indigo-100 border border-indigo-200 rounded-lg text-xs sm:text-sm font-semibold text-indigo-700 flex-shrink-0">
                ₦
              </span>
              <input
                ...
                placeholder="Min ₦100"
                className="flex-1 min-w-0 px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-lg sm:rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm transition-colors"
          ...
          <button
            ...
            disabled={payscribeLoading}
            className="w-full flex items-center justify-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 px-2 sm:px-4 bg-indigo-600 text-white rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm hover:bg-indigo-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            {payscribeLoading ? (
      ...
      {pageToggles?.enable_paymentpoint && (
        <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-indigo-200 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-500 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
          ...
          {user.virtual_account_number ? (
            <div className="space-y-2 sm:space-y-3">
              <button
                ...
                className="w-full flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-white border border-indigo-200 rounded-lg sm:rounded-xl hover:bg-indigo-50 transition-colors"
              >
              ...
              {showAccountDetails && (
                <div className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 border border-indigo-200 space-y-2 sm:space-y-3">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {copied ? <Check className="w-4 h-4 text-indigo-600" /> : <Copy className="w-4 h-4 text-gray-400" />}
                      </button>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
              ...
              <p className="text-[10px] sm:text-xs text-indigo-700 bg-indigo-100 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg">
            ...
            <button
              ...
              className="w-full py-2.5 sm:py-3 bg-indigo-600 text-white rounded-lg sm:rounded-xl font-semibold text-sm hover:bg-indigo-700 transition-colors disabled:bg-gray-300"
            >
              {generatingAccount ? (
            ...
            {transactions.filter(t => t.type?.includes('deposit')).slice(0, 5).map((txn, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                <div className="text-right">
                  <p className="font-bold text-indigo-600">+{txn.currency === 'NGN' ? '₦' : '$'}{txn.amount?.toLocaleString()}</p>
