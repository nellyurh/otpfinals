...
import { Phone, Globe, ChevronDown, RefreshCw, Copy, Check } from 'lucide-react';
import axios from 'axios';
import { toast } from 'sonner';

const API = process.env.REACT_APP_BACKEND_URL;

// Provider card data for Global Numbers
const GLOBAL_PROVIDERS = [
  {
    id: 'smspool',
    name: 'SMS Pool',
    description: 'Wide coverage with multiple countries',
    icon: '🌍',
    color: 'from-cyan-500 to-cyan-600',
    badge: 'Popular',
    features: ['100+ Countries', 'Competitive Prices', 'Fast']
  },
  {
    id: '5sim',
    name: '5sim',
...
export function GlobalNumbersSection({ user, orders, axiosConfig, fetchOrders, fetchProfile, primaryColor }) {
  ...
  const handlePurchase = async () => {
    ...
    setPurchasing(true);
    try {
      // Map provider to server format
      const serverMap = {
        'smspool': 'server1',
        '5sim': 'server2',
        'smsbower': 'smsbower_global'
      };

      const payload = {
        server: serverMap[selectedProvider.id] || selectedProvider.id,
  ...
  // Filter active global orders
  const activeOrders = orders.filter((o) => {
    if (!['smspool', '5sim', 'smsbower'].includes(o.provider)) return false;
    // Exclude US numbers (handled by US section)
    if (o.country === '187' || o.country === 'usa' || o.country === '0') return false;
  ...
  return (
    <div className="space-y-6" data-testid="global-numbers-section">
      {/* Header */}
      <div className="text-center mb-4">
        <h1 className="text-xl font-bold text-gray-900 mb-1">Global Numbers</h1>
      ...
      {activeOrders.length > 0 && (
        <div className="bg-white rounded-xl border shadow-sm p-4">
          <h3 className="font-semibold text-gray-900 mb-3">Your Global Verifications</h3>
